...
export default function Page() {
  ...
  const handleSubmit = async (formData) => {
    toast.promise(
      async () => {
        const response = await doSignUpWithCredentials(formData);
        if (response.success) {
          router.push("/login");
          return response;
        } else {
      ...
      {
        loading: "Signing up...",
        success: (data) => `User ${data.name} has been signed up successfully!`,
        error: (error) => {
          console.log(error);
          return `Sign-up failed: ${JSON.stringify(error)}`;
        },
      }
    );
  };
